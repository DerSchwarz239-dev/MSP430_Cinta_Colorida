...
void Motores_init(void){
    ...
    //Configura bot贸n en P1.2 con pull-up
    GPIO_setAsInputPinWithPullUpResistor(GPIO_PORT_P1, GPIO_PIN2);
    GPIO_clearInterrupt(GPIO_PORT_P1, GPIO_PIN2);
    //Selecciona flanco H鈫扡 (bot贸n presionado = pin LOW)
    GPIO_selectInterruptEdge(GPIO_PORT_P1, GPIO_PIN2,GPIO_HIGH_TO_LOW_TRANSITION);
    //Habilitar interrupci贸n del pin
...
void Servo_Start_PWM(void){
    ...
    /*
     * Pulso de 1ms -> Servo a 0掳
     * Pulso de 1.5ms -> Servo a 90掳
     * Pulso de 2ms -> Servo a 180掳
     */

    cpServo.compareValue = 1000;   // Arranca en 90掳 (1500us = 1.5ms)
...
void ADC_config (void){
    ...
    //Enciende n煤cleo ADC
    ADC_enable(ADC_BASE);
    //Duraci贸n de ventana de muestreo. Configuraci贸n est谩ndar.
...
void ADC_function(void){
    ...
    ADC_startConversion(ADC_BASE, ADC_SINGLECHANNEL);//Modifica velocidad seg鷑 el potenci髆etro
            if(ADC_flag){
                ADC_flag = 0;
...
__interrupt void ISR_Puerto1(void){ //Atiende el bot贸n en PI.2

   //Guarda estado de P1.2
   uint16_t status = GPIO_getInterruptStatus(BTN_SS_PORT, BTN_SS_PIN);

    if(status & GPIO_PIN2){   //Evalua que se haya presionado el bot贸n en P1.2
        //Se limpia flag de bot贸n en P1.2
        GPIO_clearInterrupt(BTN_SS_PORT, BTN_SS_PIN);
        if(Pausado){//Si est谩 pausado, prende el timer
            Pausado = 0;
            Timer_A_startCounter(TIMER_A0_BASE, TIMER_A_UP_MODE);
        }else{
            Pausado = 1;
